...
class Pdb(pdb.Pdb, ConfigurableClass, object):
    ...
    def _get_source_highlight_function(self):
        try:
            ...
            from pygments.token import String, Name, Punctuation
        except ImportError:
            return False
        ...
        class ExtendedPythonLexer(pygments.lexers.PythonLexer):
            def get_tokens_unprocessed(self, text):
                # Use the built-in PythonLexer to handle all tokens first
                string_tokens = []
                for index, token, value in super().get_tokens_unprocessed(text):
                    if token in {String.Double, String.Single}:
                        string_tokens.append((index, token, value))
                        continue
                    # Look for dictionary-like structures
                    if token is Punctuation and value == ":":
                        # Check if the preceding token was a string (likely a key)
                        yield from ((tup[0], Name.Tag, tup[2]) for tup in string_tokens)
                    else:
                        yield from string_tokens
                    string_tokens = []
                    yield index, token, value

            def _get_previous_token(self, text, index):
                """Helper function to find the preceding token."""
                tokens = list(super().get_tokens_unprocessed(text))
                for i, (idx, tok, val) in enumerate(tokens):
                    if idx >= index:
                        return tokens[i - 1] if i > 0 else None
                return None
        lexer = ExtendedPythonLexer(stripnl=False)
    ...
    def do_pp(self, arg):
        ...
        if width is None:
            try:
                ...
                width = None
        try:
            hl = self._get_source_highlight_function()
            out = io.StringIO()
            pprint.pprint(val, width=width, stream=out)
            out.seek(0)
            s = out.read()
            print(hl(s))
        except:
